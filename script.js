...
function edgePath(a, b) {
  ...
  const p1 = side(a, sA), p2 = side(b, sB);
  const d = Math.max(28, Math.min(70, Math.hypot(p2.x - p1.x, p2.y - p1.y) / 3));
  const c1 = extend(p1, sA, d), c2 = extend(p2, sB, d);
...
function startDrag(e, id) {
  ...
  if (e.target.closest(".txt.editing")) return;
...
function exportSvg() {
  ...
  parts.push(`<defs><marker id="ex-arrow" markerWidth="9" markerHeight="9" refX="7" refY="3.5" orient="auto"><path d="M0,0 L0,7 L8,3.5 z" fill="#64748b"/></marker></defs>`);
  edges.forEach(ed => {
    ...
    parts.push(`<path d="${edgePath(oa, ob)}" fill="none" stroke="#64748b" stroke-width="2" marker-end="url(#ex-arrow)"/>`);
  });
  nodes.forEach(n => {
    const off = { x: -b.minX, y: -b.minY };
...
function buildExportCanvas(scale = 2) {
  ...
  edges.forEach(ed => {
    ...
    ctx.strokeStyle = "#64748b"; ctx.lineWidth = 2; ctx.stroke(path2d);
    drawArrowHead(ctx, oa, ob);
  });
...
document.addEventListener("keydown", e => {
  ...
  else if (mod && e.key.toLowerCase() === "d" && !typing) { e.preventDefault(); duplicateSelection(); }
  else if (mod && e.key.toLowerCase() === "a" && !typing) { e.preventDefault(); selectAll(); }
  else if (mod && e.key.toLowerCase() === "c" && !typing) { copySelection(); }
  else if (mod && e.key.toLowerCase() === "x" && !typing) { e.preventDefault(); cutSelection(); }
  else if (mod && e.key.toLowerCase() === "v" && !typing) { pasteClipboard(); }
  ...
  else if (e.key === "Delete" || e.key === "Backspace") { if (!typing) { e.preventDefault(); deleteSelection(); } }
