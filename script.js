...
const wrap = $("wrap"), canvas = $("canvas"), layer = $("layer"), svg = $("svg"), empty = $("empty");
...
let hist = [], future = [];
let counter = 1;
let drag = null, resize = null, pan = null, connect = null;
let clipboard = null;
...
/* Orthogonal (org-chart style) connector: vertical drop → horizontal → vertical */
function edgePath(a, b) {
  // Prefer top-down hierarchy: leave from bottom of upper node, enter top of lower node
  ...
  if (aIsAbove) {
    // A is parent (above), B is child (below)
    p1 = side(a, "b");
    p2 = side(b, "t");
    ...
    // Side-by-side or overlapping → classic elbow from nearest sides
    const sA = near(a, center(b)), sB = near(b, center(a));
    p1 = side(a, sA);
    p2 = side(b, sB);
    // Horizontal-first or vertical-first depending on dominant direction
  ...
  // Classic org-chart path: down → across → down
  return `M ${p1.x} ${p1.y} L ${p1.x} ${midY} L ${p2.x} ${midY} L ${p2.x} ${p2.y}`;
}

/* Junction point used for the small circle in org-chart style */
function edgeJunction(a, b) {
  const aIsAbove = (a.y + a.h) <= b.y + 8;
  const bIsAbove = (b.y + b.h) <= a.y + 8;
  if (aIsAbove) {
    const p1 = side(a, "b"), p2 = side(b, "t");
    return { x: p1.x, y: Math.round((p1.y + p2.y) / 2) };
  }
  if (bIsAbove) {
    const p1 = side(a, "t"), p2 = side(b, "b");
    return { x: p2.x, y: Math.round((p1.y + p2.y) / 2) };
  }
  return null;
}
...
/* ---------------- history ---------------- */
function saveState() {
  hist.push(JSON.stringify({ nodes, edges, zoom }));
  if (hist.length > 100) hist.shift();
  future = [];
}
function restore(s) {
  const d = JSON.parse(s);
  nodes = d.nodes || []; edges = d.edges || []; zoom = d.zoom || 1;
...
function add(type, x, y) {
  saveState();
  const d = D[type];
  nodes.push({ id: "n" + counter++, type, x: snap(x - d[1] / 2), y: snap(y - d[2] / 2), w: d[1], h: d[2], text: d[0], fill: "#ffffff", border: "#334155", font: 16, line: 2 });
  selected = new Set([nodes.at(-1).id]);
  selectedEdge = null;
  render();
  status("Shape added");
}
...
function render() {
  layer.innerHTML = "";
  svg.querySelectorAll(".edge,.temp,.junction").forEach(x => x.remove());
  ...
  edges.forEach((c, idx) => {
    ...
    if (idx === selectedEdge) p.classList.add("selected");
    p.setAttribute("d", edgePath(a, b));
    p.style.pointerEvents = "stroke";
    p.onclick = ev => { ev.stopPropagation(); selectedEdge = idx; selected.clear(); render(); };
    svg.append(p);

    // Small junction circle (org-chart style)
    const j = edgeJunction(a, b);
    if (j) {
      const circ = document.createElementNS("http://www.w3.org/2000/svg", "circle");
      circ.classList.add("junction");
      if (idx === selectedEdge) circ.classList.add("selected");
      circ.setAttribute("cx", j.x);
      circ.setAttribute("cy", j.y);
...
function startDrag(e, id) {
  ...
  if (!e.shiftKey) selected = new Set([id]);
...
function startInlineEdit(n, textEl) {
  ...
  const finish = commit => {
    ...
    if (commit) {
      ...
      if (val && val !== n.text) { saveState(); n.text = val; }
    }
    render();
  };
  ...
  const onKey = e => {
    ...
    else if (e.key === "Escape") { e.preventDefault(); textEl.textContent = n.text; textEl.blur(); }
  };
...
function props() {
  ...
  $("props").classList.toggle("disabled", !n);
  $("footerHint").textContent = selected.size === 0 ? "Nothing selected"
    : selected.size === 1 ? "1 shape selected"
    : `${selected.size} shapes selected`;
  if (!n) return;
  $("pText").value = n.text; $("pW").value = n.w; $("pH").value = n.h;
  $("pX").value = n.x; $("pY").value = n.y;
  $("pFill").value = n.fill; $("pBorder").value = n.border;
  $("pFont").value = n.font; $("pLine").value = n.line;
}
function change(k, v) {
  const n = selected.size === 1 ? get([...selected][0]) : null;
  if (!n) return;
  saveState(); n[k] = v; render(); status("Updated");
}
...
$("pFill").oninput = e => { const n = selected.size === 1 ? get([...selected][0]) : null; if (n) { n.fill = e.target.value; render(); } };
$("pBorder").oninput = e => { const n = selected.size === 1 ? get([...selected][0]) : null; if (n) { n.border = e.target.value; render(); } };
...
wrap.ondrop = e => {
  ...
  if (type) { const p = pt(e); add(type, p.x, p.y); }
};
...
wrap.onpointerdown = e => {
  ...
  if (e.button === 1 || e.shiftKey) {
    ...
    selected.clear(); selectedEdge = null; render();
  }
};
...
function renderMinimap() {
  ...
  const minX = Math.min(...nodes.map(n => n.x), wrap.scrollLeft / zoom);
  const maxX = Math.max(...nodes.map(n => n.x + n.w), (wrap.scrollLeft + wrap.clientWidth) / zoom);
  const minY = Math.min(...nodes.map(n => n.y), wrap.scrollTop / zoom);
  const maxY = Math.max(...nodes.map(n => n.y + n.h), (wrap.scrollTop + wrap.clientHeight) / zoom);
...
function selectAll() { selected = new Set(nodes.map(n => n.id)); selectedEdge = null; render(); status("Selected all"); }
...
function newDiagram() {
  ...
  saveState();
  nodes = []; edges = []; selected.clear(); selectedEdge = null;
...
$("file").onchange = e => {
  ...
  reader.onload = () => {
    try {
      ...
      selected.clear(); selectedEdge = null;
      render(); fitToScreen();
      status("Diagram opened");
    } catch { alert("Couldn't read that file — is it a FlowChart Studio JSON export?"); }
  };
  reader.readAsText(f);
  e.target.value = "";
};
...
function exportJson() {
  download("flowchart.json", new Blob([JSON.stringify({ nodes, edges }, null, 2)], { type: "application/json" }));
  status("JSON exported");
}
function diagramBounds(pad = 40) {
  if (!nodes.length) return { minX: 0, minY: 0, w: 400, h: 300 };
...
function exportSvg() {
  ...
  edges.forEach(ed => {
    const a = get(ed.a), b2 = get(ed.b);
    if (!a || !b2) return;
    const off = { x: -b.minX, y: -b.minY };
    const oa = { ...a, x: a.x + off.x, y: a.y + off.y }, ob = { ...b2, x: b2.x + off.x, y: b2.y + off.y };
    parts.push(`<path d="${edgePath(oa, ob)}" fill="none" stroke="#94a3b8" stroke-width="1.75"/>`);
    const j = edgeJunction(oa, ob);
    if (j) parts.push(`<circle cx="${j.x}" cy="${j.y}" r="5" fill="#fff" stroke="#94a3b8" stroke-width="1.5"/>`);
  });
  nodes.forEach(n => {
    const off = { x: -b.minX, y: -b.minY };
    const g = shapeGeom({ ...n, x: n.x + off.x, y: n.y + off.y });
    ...
    else parts.push(`<rect x="${n.x + off.x}" y="${n.y + off.y}" width="${n.w}" height="${n.h}" rx="${g.rx}" ${style}/>`);
    parts.push(`<text x="${n.x + off.x + n.w / 2}" y="${n.y + off.y + n.h / 2}" font-size="${n.font}" fill="#1a2233" text-anchor="middle" dominant-baseline="middle">${escapeXml(n.text)}</text>`);
  });
  parts.push("</svg>");
...
function buildExportCanvas(scale = 2) {
  ...
  c.width = Math.max(1, b.w * scale); c.height = Math.max(1, b.h * scale);
  const ctx = c.getContext("2d");
  ctx.scale(scale, scale);
  ctx.fillStyle = "#ffffff"; ctx.fillRect(0, 0, b.w, b.h);
  const off = { x: -b.minX, y: -b.minY };
  ...
  edges.forEach(ed => {
    ...
    if (!a || !b2) return;
    const oa = { ...a, x: a.x + off.x, y: a.y + off.y }, ob = { ...b2, x: b2.x + off.x, y: b2.y + off.y };
    const path2d = new Path2D(edgePath(oa, ob));
    ctx.strokeStyle = "#94a3b8"; ctx.lineWidth = 1.75; ctx.stroke(path2d);
    // Junction circle (org-chart style)
    const j = edgeJunction(oa, ob);
    if (j) {
      ctx.beginPath();
  ...
  nodes.forEach(n => {
    const nn = { ...n, x: n.x + off.x, y: n.y + off.y };
    ...
    ctx.fillText(n.text, nn.x + nn.w / 2, nn.y + nn.h / 2, nn.w - 16);
  });
...
function exportPng() {
  ...
  c.toBlob(blob => { download("flowchart.png", blob); status("PNG exported"); });
}
function exportPdf() {
  ...
  const pdf = new jsPDF({ orientation, unit: "px", format: [Math.max(b.w, 1), Math.max(b.h, 1)] });
...
<head><meta charset="utf-8"><title>Flowchart</title></head>
<body><h2 style="font-family:Calibri,Arial,sans-serif">Flowchart</h2>
<img src="${imgData}" style="max-width:100%"/></body></html>`;
  download("flowchart.doc", new Blob(["\ufeff", html], { type: "application/msword" }));
  status("Word document exported");
}
...
const actions = {
  new: newDiagram, open: openDiagram, save: saveLocal,
...
document.addEventListener("keydown", e => {
  ...
  else if (e.key === "Escape") { selected.clear(); selectedEdge = null; render(); }
...
/* ---------------- autosave + boot ---------------- */
setInterval(() => localStorage.setItem("flowchartV2", JSON.stringify({ nodes, edges, zoom })), 5000);
